  ...
  <div class="endpoints-page">
    ...
    <div class="page-header-gradient">
      <div class="header-left">
        ...
        <span class="page-subtitle">管理集群中的端点资源</span>
      </div>
    ...
    <div class="glass-table-container">
      ...
      >
        ...
        <el-table-column label="地址" min-width="200">
          <template #default="{ row }">
            <el-tag v-for="addr in (row.addresses || [])" :key="addr" size="small" style="margin: 2px">{{ addr }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="端口" min-width="200">
          <template #default="{ row }">
            <div v-for="port in (row.ports || [])" :key="port.port">
              <span class="cell-metric">{{ port.name }}: {{ port.port }}/{{ port.protocol }}</span>
            </div>
          </template>
        </el-table-column>
...
const fetchData = async () => {
  ...
  try {
    ...
    const res: any = await api.get('/endpoints', { params })
