  ...
  <div class="page-container">
    <div class="page-header-gradient">
      <div class="header-left">
        <h1 class="page-title">ClusterRoleBindings</h1>
        <span class="page-subtitle">管理集群级别的角色绑定</span>
      </div>
      <div class="header-actions">
        <button type="button" class="btn-gradient" @click="showCreate">
          ...
          <span>新建 ClusterRoleBinding</span>
        </button>
    ...
    <div class="glass-table-container">
      <el-table
        ...
        class="custom-table"
        :empty-text="'暂无 ClusterRoleBinding 数据'"
      >
    ...
    <!-- Create Dialog -->
    <el-dialog v-model="createVisible" title="新建 ClusterRoleBinding" width="650px" :close-on-click-modal="false" class="dark-dialog">
      <div class="dialog-form">
        <div class="form-row">
          <label class="form-label">名称</label>
          <el-input v-model="createForm.name" placeholder="my-clusterrolebinding" />
        </div>
        <div class="form-row">
        ...
        <div class="form-row">
          ...
          <el-input v-model="createForm.roleRefName" placeholder="clusterrole-name" />
        </div>
...
const fetchData = async () => {
  loading.value = true
  try {
    const res: any = await api.get('/clusterrolebindings')
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm(`确定要删除 ClusterRoleBinding ${row.name} 吗？`, '确认', { type: 'warning' })
    await api.delete(`/clusterrolebindings/delete?name=${row.name}`)
...
const submitCreate = async () => {
  if (!createForm.value.name || !createForm.value.roleRefName) {
    ElMessage.warning('请填写名称和角色引用名称')
    return
  }
  ...
  try {
    ...
    await api.post('/clusterrolebindings/create', {
