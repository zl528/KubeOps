<template>
  <div class="configmaps-page">
    <div class="page-header-gradient">
      <div class="header-left">
        <h1 class="page-title">ConfigMaps</h1>
        <span class="page-subtitle">管理集群中的配置映射资源</span>
      </div>
    ...
    <div class="glass-table-container">
      ...
      >
        ...
        <el-table-column label="数据键" min-width="250">
          <template #default="{ row }">
            <el-tag v-for="(_, k) in (row.data || {})" :key="k" size="small" style="margin: 2px">{{ k }}</el-tag>
          </template>
        </el-table-column>
    ...
    <!-- 查看详情 -->
    <el-dialog v-model="detailVisible" title="ConfigMap 详情" width="700px" class="dark-dialog">
      <template v-if="selected">
        ...
        <div v-for="(val, key) in (selected.data || {})" :key="key" class="data-item">
          <div class="data-key">{{ key }}</div>
          <pre class="data-value">{{ val }}</pre>
        </div>
        <el-empty v-if="!selected.data || Object.keys(selected.data).length === 0" description="无数据" />
      </template>
    </el-dialog>

    <!-- 创建/编辑 -->
    <el-dialog v-model="editVisible" :title="isEdit ? '编辑 ConfigMap' : '新建 ConfigMap'" width="750px" class="dark-dialog">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="名称">
          <el-input v-model="form.name" :disabled="isEdit" placeholder="my-configmap" />
        </el-form-item>
        ...
        <div v-for="(item, idx) in form.items" :key="idx" class="kv-row">
          <el-input v-model="item.key" placeholder="键" style="width: 200px" />
          <el-input v-model="item.value" type="textarea" :autosize="{ minRows: 1, maxRows: 8 }" placeholder="值" style="flex: 1" />
...
const fetchData = async () => {
  ...
  try {
    ...
    const res: any = await api.get('/configmaps', { params })
...
const viewDetail = async (row: any) => {
  try {
    const res: any = await api.get('/configmaps/get', { params: { namespace: row.namespace, name: row.name } })
...
const editItem = (row: any) => {
  ...
  form.name = row.name
  form.items = Object.entries(row.data || {}).map(([k, v]) => ({ key: k, value: v as string }))
...
const save = async () => {
  ...
  try {
    if (isEdit.value) {
      await api.post('/configmaps/update', { namespace: form.namespace, name: form.name, data, labels, annotations })
      ElMessage.success('更新成功')
    } else {
      await api.post('/configmaps/create', { namespace: form.namespace, name: form.name, data, labels, annotations })
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm(`确定要删除 ConfigMap "${row.name}" 吗？`, '确认', { type: 'warning' })
    await api.delete(`/configmaps/delete?namespace=${row.namespace}&name=${row.name}`)
...
.configmaps-page :deep(.el-dialog) {
...
.configmaps-page :deep(.el-dialog__header) {
  border-bottom: 1px solid rgba(148, 163, 184, 0.08);
  padding: 20px 24px;
  margin: 0;
}

.configmaps-page :deep(.el-dialog__title) {
  color: var(--text-primary);
  font-weight: 600;
  font-size: 18px;
}

.configmaps-page :deep(.el-dialog__headerbtn .el-dialog__close) {
  color: var(--text-secondary);
}

.configmaps-page :deep(.el-dialog__body) {
  padding: 24px;
  color: var(--text-primary);
}

.configmaps-page :deep(.el-dialog__footer) {
  border-top: 1px solid rgba(148, 163, 184, 0.08);
  padding: 16px 24px;
}

.configmaps-page :deep(.el-descriptions) {
  --el-descriptions-item-bordered-label-background: rgba(30, 41, 59, 0.8);
}

.configmaps-page :deep(.el-descriptions__label) {
  color: var(--text-secondary);
}

.configmaps-page :deep(.el-descriptions__content) {
  color: var(--text-primary);
}

.configmaps-page :deep(.el-descriptions__cell) {
  border-color: rgba(148, 163, 184, 0.08) !important;
}

.configmaps-page :deep(.el-divider__text) {
  background: #1e293b;
  color: var(--text-secondary);
}

.configmaps-page :deep(.el-divider) {
  border-color: rgba(148, 163, 184, 0.08);
}

.configmaps-page :deep(.el-form-item__label) {
  color: var(--text-secondary);
}

.configmaps-page :deep(.el-input__wrapper) {
  background: rgba(30, 41, 59, 0.8);
  border: 1px solid rgba(148, 163, 184, 0.1);
  box-shadow: none;
}

.configmaps-page :deep(.el-input__wrapper:hover) {
  border-color: rgba(99, 102, 241, 0.4);
}

.configmaps-page :deep(.el-input__wrapper.is-focus) {
  border-color: #6366f1;
  box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.15);
}

.configmaps-page :deep(.el-input__inner) {
  color: var(--text-primary);
}

.configmaps-page :deep(.el-input__inner::placeholder) {
  color: var(--text-secondary);
}

.configmaps-page :deep(.el-textarea__inner) {
  background: rgba(30, 41, 59, 0.8);
  border: 1px solid rgba(148, 163, 184, 0.1);
  color: var(--text-primary);
  box-shadow: none;
...
.configmaps-page :deep(.el-textarea__inner:focus) {
  border-color: #6366f1;
  box-shadow: 0 0 0 2px rgba(99, 102, 241, 0.15);
}

.configmaps-page :deep(.el-tag--info) {
  background: rgba(51, 65, 85, 0.6);
  border-color: rgba(148, 163, 184, 0.1);
  color: var(--text-secondary);
}

.configmaps-page :deep(.el-empty__description p) {
  color: var(--text-secondary);
}

.configmaps-page :deep(.el-overlay) {
