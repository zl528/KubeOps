  ...
  <div class="page-container">
    <div class="page-header-gradient">
      <div class="header-left">
        <h1 class="page-title">ClusterRoles</h1>
        <span class="page-subtitle">管理集群级别的角色权限</span>
      </div>
    ...
    <!-- Edit Dialog -->
    <el-dialog v-model="editVisible" title="编辑 ClusterRole" width="700px" :close-on-click-modal="false" class="dark-dialog">
    ...
    <!-- Create Dialog -->
    <el-dialog v-model="createVisible" title="新建 ClusterRole" width="700px" :close-on-click-modal="false" class="dark-dialog">
      <div class="dialog-form">
        <div class="form-row">
          <label class="form-label">名称</label>
          <el-input v-model="createForm.name" placeholder="my-clusterrole" />
        </div>
...
const fetchData = async () => {
  loading.value = true
  try {
    const res: any = await api.get('/clusterroles')
...
const openEditDialog = async (row: any) => {
  try {
    const res: any = await api.get(`/clusterroles/get?name=${row.name}`)
    const cr = res.data
    editForm.value = {
      name: cr.name,
      rules: (cr.rules || []).map((r: any) => ({
    ...
    ElMessage.error('获取 ClusterRole 详情失败')
  }
}
...
const submitCreate = async () => {
  ...
  try {
    ...
    await api.post('/clusterroles/create', {
...
const submitEdit = async () => {
  ...
  try {
    ...
    await api.post('/clusterroles/update', {
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm(`确定要删除 ClusterRole ${row.name} 吗？`, '确认', { type: 'warning' })
    await api.delete(`/clusterroles/delete?name=${row.name}`)
