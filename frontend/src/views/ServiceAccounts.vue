  ...
  <div class="page-container">
    <div class="page-header-gradient">
      <div class="header-left">
        <h1 class="page-title">ServiceAccounts</h1>
        <span class="page-subtitle">管理命名空间中的服务账户</span>
      </div>
    ...
    <div class="glass-table-container">
      <el-table
        ...
        class="custom-table"
        :empty-text="'暂无 ServiceAccount 数据'"
      >
        ...
        <el-table-column label="标签" min-width="200">
          <template #default="{ row }">
            <el-tag v-for="(v, k) in (row.labels || {}).slice?.(0, 3)" :key="k" size="small" style="margin: 2px">
              {{ k }}={{ v }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <!-- Create Dialog -->
    <el-dialog v-model="createVisible" title="新建 ServiceAccount" width="600px" :close-on-click-modal="false" class="dark-dialog">
      <div class="dialog-form">
        ...
        <div class="form-row">
          ...
          <el-input v-model="createForm.name" placeholder="my-serviceaccount" />
        </div>
        ...
        <div class="form-row">
          <label class="form-label">自动挂载 API Token</label>
          <el-switch v-model="createForm.automountServiceAccountToken" active-text="是" inactive-text="否" />
        </div>
        <div class="kv-section">
          <div class="rules-header">
            <label class="form-label">镜像拉取密钥 (ImagePullSecrets)</label>
            <button type="button" class="btn-gradient btn-small" @click="addCreateImagePullSecret">+ 添加密钥</button>
          </div>
          <div v-for="(sec, idx) in createForm.imagePullSecrets" :key="idx" class="kv-row">
            <el-input v-model="sec.name" placeholder="secret-name" class="kv-input" />
            <button type="button" class="btn-remove" @click="removeCreateImagePullSecret(idx)">删除</button>
          </div>
        </div>
      </div>
      <template #footer>
...
const fetchData = async () => {
  ...
  try {
    ...
    const res: any = await api.get('/serviceaccounts', { params })
...
const addCreateImagePullSecret = () => {
  createForm.value.imagePullSecrets.push({ name: '' })
}

const removeCreateImagePullSecret = (idx: number) => {
  createForm.value.imagePullSecrets.splice(idx, 1)
}
...
const handleDelete = async (row: any) => {
  try {
    await ElMessageBox.confirm(`确定要删除 ServiceAccount ${row.name} 吗？`, '确认', { type: 'warning' })
    await api.delete(`/serviceaccounts/delete?namespace=${row.namespace}&name=${row.name}`)
...
const submitCreate = async () => {
  ...
  try {
    ...
    const imagePullSecrets = createForm.value.imagePullSecrets
      .filter(s => s.name)
      .map(s => ({ name: s.name }))
    await api.post('/serviceaccounts/create', {
      namespace: createForm.value.namespace,
      name: createForm.value.name,
      labels,
      annotations,
      automountServiceAccountToken: createForm.value.automountServiceAccountToken,
      imagePullSecrets
    })
